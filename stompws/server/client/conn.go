		...
		case sub, ok := <-c.subChannel:
			...
			if _, ok = c.subs[sub.id]; ok {
				...
				if sub.ack == frame.AckAuto {
					...
					// straight away
					sub.frame = nil
					c.requestChannel <- Request{Op: SubscribeOp, Conn: c, Sub: sub}
				} else {
				...
				c.requestChannel <- Request{Op: RequeueOp, Conn: c, Frame: sub.frame}
			}
...
func (c *Conn) cleanupConn() {
	...
	for _, sub := range c.subs {
		...
		c.requestChannel <- Request{Op: UnsubscribeOp, Conn: c, Sub: sub}
	}
...
func (c *Conn) handleAck(f *frame.Frame) error {
	...
	if tx, ok := f.Header.Contains(frame.Transaction); ok {
		...
		c.subList.Ack(msgId64, func(s *Subscription) {
			...
			c.requestChannel <- Request{Op: SubscribeOp, Conn: c, Sub: s}
		})
	}
...
func (c *Conn) handleNack(f *frame.Frame) error {
	...
	if tx, ok := f.Header.Contains(frame.Transaction); ok {
		...
		c.subList.Nack(msgId64, func(s *Subscription) {
			...
			c.requestChannel <- Request{Op: RequeueOp, Conn: c, Frame: s.frame}
...
func (c *Conn) handleSend(f *frame.Frame) error {
	...
	if tx, ok := f.Header.Contains(frame.Transaction); ok {
		...
		c.requestChannel <- Request{Op: EnqueueOp, Conn: c, Frame: f}
	}
