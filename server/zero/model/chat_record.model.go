...
type (
	// 接口定义
	IChatRecordModel interface {
		// 切换事务操作
		WithTransaction(tx *gorm.DB) (out IChatRecordModel)
		// 增删改查
...
func NewChatRecordModel(db *gorm.DB, cache *redis.Client) IChatRecordModel {
...
// 切换事务操作
func (s *defaultChatRecordModel) WithTransaction(tx *gorm.DB) (out IChatRecordModel) {
...
// 删除ChatRecord记录
func (s *defaultChatRecordModel) Delete(ctx context.Context, id int) (rows int64, err error) {
	db := s.DbEngin.WithContext(ctx).Table(s.tableName)
...
// 分页查询ChatRecord记录
func (s *defaultChatRecordModel) FindList(ctx context.Context, page int, size int, sorts string, conditions string, args ...interface{}) (list []*ChatRecord, err error) {
	// 创建db
	db := s.DbEngin.WithContext(ctx).Table(s.tableName)
