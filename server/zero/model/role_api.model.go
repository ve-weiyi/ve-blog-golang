...
type (
	// 接口定义
	IRoleApiModel interface {
		// 切换事务操作
		WithTransaction(tx *gorm.DB) (out IRoleApiModel)
		// 增删改查
...
func NewRoleApiModel(db *gorm.DB, cache *redis.Client) IRoleApiModel {
...
// 切换事务操作
func (s *defaultRoleApiModel) WithTransaction(tx *gorm.DB) (out IRoleApiModel) {
...
// 分页查询RoleApi记录
func (s *defaultRoleApiModel) FindList(ctx context.Context, page int, size int, sorts string, conditions string, args ...interface{}) (list []*RoleApi, err error) {
	// 创建db
	db := s.DbEngin.WithContext(ctx).Table(s.tableName)
