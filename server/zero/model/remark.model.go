...
type (
	// 接口定义
	IRemarkModel interface {
		// 切换事务操作
		WithTransaction(tx *gorm.DB) (out IRemarkModel)
		// 增删改查
...
func NewRemarkModel(db *gorm.DB, cache *redis.Client) IRemarkModel {
...
// 切换事务操作
func (s *defaultRemarkModel) WithTransaction(tx *gorm.DB) (out IRemarkModel) {
...
// 删除Remark记录
func (s *defaultRemarkModel) Delete(ctx context.Context, id int) (rows int64, err error) {
	db := s.DbEngin.WithContext(ctx).Table(s.tableName)
...
// 分页查询Remark记录
func (s *defaultRemarkModel) FindList(ctx context.Context, page int, size int, sorts string, conditions string, args ...interface{}) (list []*Remark, err error) {
	// 创建db
	db := s.DbEngin.WithContext(ctx).Table(s.tableName)
