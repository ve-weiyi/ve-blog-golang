...
type (
	// 接口定义
	IUniqueViewModel interface {
		// 切换事务操作
		WithTransaction(tx *gorm.DB) (out IUniqueViewModel)
		// 增删改查
...
func NewUniqueViewModel(db *gorm.DB, cache *redis.Client) IUniqueViewModel {
...
// 切换事务操作
func (s *defaultUniqueViewModel) WithTransaction(tx *gorm.DB) (out IUniqueViewModel) {
...
// 分页查询UniqueView记录
func (s *defaultUniqueViewModel) FindList(ctx context.Context, page int, size int, sorts string, conditions string, args ...interface{}) (list []*UniqueView, err error) {
	// 创建db
	db := s.DbEngin.WithContext(ctx).Table(s.tableName)
