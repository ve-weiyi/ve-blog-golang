...
type (
	// 接口定义
	IUserOauthModel interface {
		// 切换事务操作
		WithTransaction(tx *gorm.DB) (out IUserOauthModel)
		// 增删改查
...
func NewUserOauthModel(db *gorm.DB, cache *redis.Client) IUserOauthModel {
...
// 切换事务操作
func (s *defaultUserOauthModel) WithTransaction(tx *gorm.DB) (out IUserOauthModel) {
...
// 分页查询UserOauth记录
func (s *defaultUserOauthModel) FindList(ctx context.Context, page int, size int, sorts string, conditions string, args ...interface{}) (list []*UserOauth, err error) {
	// 创建db
	db := s.DbEngin.WithContext(ctx).Table(s.tableName)
