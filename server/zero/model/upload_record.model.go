...
type (
	// 接口定义
	IUploadRecordModel interface {
		// 切换事务操作
		WithTransaction(tx *gorm.DB) (out IUploadRecordModel)
		// 增删改查
...
func NewUploadRecordModel(db *gorm.DB, cache *redis.Client) IUploadRecordModel {
...
// 切换事务操作
func (s *defaultUploadRecordModel) WithTransaction(tx *gorm.DB) (out IUploadRecordModel) {
...
// 分页查询UploadRecord记录
func (s *defaultUploadRecordModel) FindList(ctx context.Context, page int, size int, sorts string, conditions string, args ...interface{}) (list []*UploadRecord, err error) {
	// 创建db
	db := s.DbEngin.WithContext(ctx).Table(s.tableName)
