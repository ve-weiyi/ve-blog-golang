...
type (
	// 接口定义
	IUserInformationModel interface {
		// 切换事务操作
		WithTransaction(tx *gorm.DB) (out IUserInformationModel)
		// 增删改查
...
func NewUserInformationModel(db *gorm.DB, cache *redis.Client) IUserInformationModel {
...
// 切换事务操作
func (s *defaultUserInformationModel) WithTransaction(tx *gorm.DB) (out IUserInformationModel) {
...
// 删除UserInformation记录
func (s *defaultUserInformationModel) Delete(ctx context.Context, id int) (rows int64, err error) {
	db := s.DbEngin.WithContext(ctx).Table(s.tableName)
...
func (s *defaultUserInformationModel) FindList(ctx context.Context, page int, size int, sorts string, conditions string, args ...interface{}) (list []*UserInformation, err error) {
	// 创建db
	db := s.DbEngin.WithContext(ctx).Table(s.tableName)
