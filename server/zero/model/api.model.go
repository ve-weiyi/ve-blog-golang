...
type (
	...
	IApiModel interface {
		// 切换事务操作
		WithTransaction(tx *gorm.DB) (out IApiModel)
		// 增删改查
...
func NewApiModel(db *gorm.DB, cache *redis.Client) IApiModel {
...
// 切换事务操作
func (s *defaultApiModel) WithTransaction(tx *gorm.DB) (out IApiModel) {
...
// 分页查询Api记录
func (s *defaultApiModel) FindList(ctx context.Context, page int, size int, sorts string, conditions string, args ...interface{}) (list []*Api, err error) {
	// 创建db
	db := s.DbEngin.WithContext(ctx).Table(s.tableName)
