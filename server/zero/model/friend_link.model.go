...
type (
	// 接口定义
	IFriendLinkModel interface {
		// 切换事务操作
		WithTransaction(tx *gorm.DB) (out IFriendLinkModel)
		// 增删改查
...
func NewFriendLinkModel(db *gorm.DB, cache *redis.Client) IFriendLinkModel {
...
// 切换事务操作
func (s *defaultFriendLinkModel) WithTransaction(tx *gorm.DB) (out IFriendLinkModel) {
...
// 删除FriendLink记录
func (s *defaultFriendLinkModel) Delete(ctx context.Context, id int) (rows int64, err error) {
	db := s.DbEngin.WithContext(ctx).Table(s.tableName)
...
// 分页查询FriendLink记录
func (s *defaultFriendLinkModel) FindList(ctx context.Context, page int, size int, sorts string, conditions string, args ...interface{}) (list []*FriendLink, err error) {
	// 创建db
	db := s.DbEngin.WithContext(ctx).Table(s.tableName)
