...
type (
	...
	IPhotoAlbumModel interface {
		// 切换事务操作
		WithTransaction(tx *gorm.DB) (out IPhotoAlbumModel)
		// 增删改查
...
func NewPhotoAlbumModel(db *gorm.DB, cache *redis.Client) IPhotoAlbumModel {
...
// 切换事务操作
func (s *defaultPhotoAlbumModel) WithTransaction(tx *gorm.DB) (out IPhotoAlbumModel) {
...
// 分页查询PhotoAlbum记录
func (s *defaultPhotoAlbumModel) FindList(ctx context.Context, page int, size int, sorts string, conditions string, args ...interface{}) (list []*PhotoAlbum, err error) {
	// 创建db
	db := s.DbEngin.WithContext(ctx).Table(s.tableName)
