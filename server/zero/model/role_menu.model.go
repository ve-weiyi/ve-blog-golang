...
type (
	// 接口定义
	IRoleMenuModel interface {
		// 切换事务操作
		WithTransaction(tx *gorm.DB) (out IRoleMenuModel)
		// 增删改查
...
func NewRoleMenuModel(db *gorm.DB, cache *redis.Client) IRoleMenuModel {
...
// 切换事务操作
func (s *defaultRoleMenuModel) WithTransaction(tx *gorm.DB) (out IRoleMenuModel) {
...
// 删除RoleMenu记录
func (s *defaultRoleMenuModel) Delete(ctx context.Context, id int) (rows int64, err error) {
	db := s.DbEngin.WithContext(ctx).Table(s.tableName)
...
// 分页查询RoleMenu记录
func (s *defaultRoleMenuModel) FindList(ctx context.Context, page int, size int, sorts string, conditions string, args ...interface{}) (list []*RoleMenu, err error) {
	// 创建db
	db := s.DbEngin.WithContext(ctx).Table(s.tableName)
