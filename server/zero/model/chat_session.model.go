...
type (
	// 接口定义
	IChatSessionModel interface {
		// 切换事务操作
		WithTransaction(tx *gorm.DB) (out IChatSessionModel)
		// 增删改查
...
func NewChatSessionModel(db *gorm.DB, cache *redis.Client) IChatSessionModel {
...
// 切换事务操作
func (s *defaultChatSessionModel) WithTransaction(tx *gorm.DB) (out IChatSessionModel) {
...
// 分页查询ChatSession记录
func (s *defaultChatSessionModel) FindList(ctx context.Context, page int, size int, sorts string, conditions string, args ...interface{}) (list []*ChatSession, err error) {
	// 创建db
	db := s.DbEngin.WithContext(ctx).Table(s.tableName)
