...
type (
	// 接口定义
	ITalkModel interface {
		// 切换事务操作
		WithTransaction(tx *gorm.DB) (out ITalkModel)
		// 增删改查
...
func NewTalkModel(db *gorm.DB, cache *redis.Client) ITalkModel {
...
// 切换事务操作
func (s *defaultTalkModel) WithTransaction(tx *gorm.DB) (out ITalkModel) {
...
// 删除Talk记录
func (s *defaultTalkModel) Delete(ctx context.Context, id int) (rows int64, err error) {
	db := s.DbEngin.WithContext(ctx).Table(s.tableName)
...
// 分页查询Talk记录
func (s *defaultTalkModel) FindList(ctx context.Context, page int, size int, sorts string, conditions string, args ...interface{}) (list []*Talk, err error) {
	// 创建db
	db := s.DbEngin.WithContext(ctx).Table(s.tableName)
