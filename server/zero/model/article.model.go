...
type (
	// 接口定义
	IArticleModel interface {
		// 切换事务操作
		WithTransaction(tx *gorm.DB) (out IArticleModel)
		// 增删改查
...
func NewArticleModel(db *gorm.DB, cache *redis.Client) IArticleModel {
...
// 切换事务操作
func (s *defaultArticleModel) WithTransaction(tx *gorm.DB) (out IArticleModel) {
...
// 分页查询Article记录
func (s *defaultArticleModel) FindList(ctx context.Context, page int, size int, sorts string, conditions string, args ...interface{}) (list []*Article, err error) {
	// 创建db
	db := s.DbEngin.WithContext(ctx).Table(s.tableName)
