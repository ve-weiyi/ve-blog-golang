...
type (
	// 接口定义
	IUserRoleModel interface {
		// 切换事务操作
		WithTransaction(tx *gorm.DB) (out IUserRoleModel)
		// 增删改查
...
func NewUserRoleModel(db *gorm.DB, cache *redis.Client) IUserRoleModel {
...
// 切换事务操作
func (s *defaultUserRoleModel) WithTransaction(tx *gorm.DB) (out IUserRoleModel) {
...
// 删除UserRole记录
func (s *defaultUserRoleModel) Delete(ctx context.Context, id int) (rows int64, err error) {
	db := s.DbEngin.WithContext(ctx).Table(s.tableName)
...
// 分页查询UserRole记录
func (s *defaultUserRoleModel) FindList(ctx context.Context, page int, size int, sorts string, conditions string, args ...interface{}) (list []*UserRole, err error) {
	// 创建db
	db := s.DbEngin.WithContext(ctx).Table(s.tableName)
