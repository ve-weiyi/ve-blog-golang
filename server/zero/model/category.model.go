...
type (
	// 接口定义
	ICategoryModel interface {
		// 切换事务操作
		WithTransaction(tx *gorm.DB) (out ICategoryModel)
		// 增删改查
...
func NewCategoryModel(db *gorm.DB, cache *redis.Client) ICategoryModel {
...
// 切换事务操作
func (s *defaultCategoryModel) WithTransaction(tx *gorm.DB) (out ICategoryModel) {
...
// 分页查询Category记录
func (s *defaultCategoryModel) FindList(ctx context.Context, page int, size int, sorts string, conditions string, args ...interface{}) (list []*Category, err error) {
	// 创建db
	db := s.DbEngin.WithContext(ctx).Table(s.tableName)
