...
type (
	// 接口定义
	IPhotoModel interface {
		// 切换事务操作
		WithTransaction(tx *gorm.DB) (out IPhotoModel)
		// 增删改查
...
func NewPhotoModel(db *gorm.DB, cache *redis.Client) IPhotoModel {
...
// 切换事务操作
func (s *defaultPhotoModel) WithTransaction(tx *gorm.DB) (out IPhotoModel) {
...
// 分页查询Photo记录
func (s *defaultPhotoModel) FindList(ctx context.Context, page int, size int, sorts string, conditions string, args ...interface{}) (list []*Photo, err error) {
	// 创建db
	db := s.DbEngin.WithContext(ctx).Table(s.tableName)
