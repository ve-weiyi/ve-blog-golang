...
type (
	// 接口定义
	IRoleModel interface {
		// 切换事务操作
		WithTransaction(tx *gorm.DB) (out IRoleModel)
		// 增删改查
	...
	Role struct {
		...
		RolePID     int64  `json:"role_pid"`     // 父角色id
...
func NewRoleModel(db *gorm.DB, cache *redis.Client) IRoleModel {
...
// 切换事务操作
func (s *defaultRoleModel) WithTransaction(tx *gorm.DB) (out IRoleModel) {
...
// 分页查询Role记录
func (s *defaultRoleModel) FindList(ctx context.Context, page int, size int, sorts string, conditions string, args ...interface{}) (list []*Role, err error) {
	// 创建db
	db := s.DbEngin.WithContext(ctx).Table(s.tableName)
