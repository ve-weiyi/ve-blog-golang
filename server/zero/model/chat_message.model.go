...
type (
	// 接口定义
	IChatMessageModel interface {
		// 切换事务操作
		WithTransaction(tx *gorm.DB) (out IChatMessageModel)
		// 增删改查
...
func NewChatMessageModel(db *gorm.DB, cache *redis.Client) IChatMessageModel {
...
// 切换事务操作
func (s *defaultChatMessageModel) WithTransaction(tx *gorm.DB) (out IChatMessageModel) {
...
// 分页查询ChatMessage记录
func (s *defaultChatMessageModel) FindList(ctx context.Context, page int, size int, sorts string, conditions string, args ...interface{}) (list []*ChatMessage, err error) {
	// 创建db
	db := s.DbEngin.WithContext(ctx).Table(s.tableName)
