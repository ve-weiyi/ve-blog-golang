...
type (
	// 接口定义
	IUserLoginHistoryModel interface {
		// 切换事务操作
		WithTransaction(tx *gorm.DB) (out IUserLoginHistoryModel)
		// 增删改查
...
func NewUserLoginHistoryModel(db *gorm.DB, cache *redis.Client) IUserLoginHistoryModel {
...
// 切换事务操作
func (s *defaultUserLoginHistoryModel) WithTransaction(tx *gorm.DB) (out IUserLoginHistoryModel) {
...
func (s *defaultUserLoginHistoryModel) FindList(ctx context.Context, page int, size int, sorts string, conditions string, args ...interface{}) (list []*UserLoginHistory, err error) {
	// 创建db
	db := s.DbEngin.WithContext(ctx).Table(s.tableName)
