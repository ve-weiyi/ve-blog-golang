...
// 分页查询CasbinRule记录
func (s *CasbinRuleRepository) FindCasbinRuleList(ctx context.Context, page *request.PageQuery) (list []*entity.CasbinRule, err error) {
	// 创建db
	db := s.DbEngin.WithContext(ctx)
...
// 查询总数
func (s *CasbinRuleRepository) Count(ctx context.Context, conditions ...*request.Condition) (count int64, err error) {
	db := s.DbEngin.WithContext(ctx)
