...
// 分页查询Menu记录
func (s *MenuRepository) FindMenuList(ctx context.Context, page *request.PageQuery) (list []*entity.Menu, err error) {
	// 创建db
	db := s.DbEngin.WithContext(ctx)
...
// 查询总数
func (s *MenuRepository) Count(ctx context.Context, conditions ...*request.Condition) (count int64, err error) {
	db := s.DbEngin.WithContext(ctx)
