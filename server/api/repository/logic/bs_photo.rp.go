...
// 分页查询Photo记录
func (s *PhotoRepository) FindPhotoList(ctx context.Context, page *request.PageQuery) (list []*entity.Photo, err error) {
	// 创建db
	db := s.DbEngin.WithContext(ctx)
...
// 查询总数
func (s *PhotoRepository) Count(ctx context.Context, conditions ...*request.Condition) (count int64, err error) {
	db := s.DbEngin.WithContext(ctx)
