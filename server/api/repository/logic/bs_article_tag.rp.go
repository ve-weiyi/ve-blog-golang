...
// 分页查询ArticleTag记录
func (s *ArticleTagRepository) FindArticleTagList(ctx context.Context, page *request.PageQuery) (list []*entity.ArticleTag, err error) {
	// 创建db
	db := s.DbEngin.WithContext(ctx)
...
// 查询总数
func (s *ArticleTagRepository) Count(ctx context.Context, conditions ...*request.Condition) (count int64, err error) {
	db := s.DbEngin.WithContext(ctx)
