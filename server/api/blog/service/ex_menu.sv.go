	...
	for _, item := range req.Menus {
		...
		if exist == nil {
			...
			exist = &entity.Menu{
				...
				Path:      item.Path,
				Name:      item.Name,
				Component: jsonconv.AnyToJsonNE(item.Component),
				Redirect:  item.Redirect,
				Type:      item.Type,
				Extra:     jsonconv.AnyToJsonNE(item.Meta),
			}
		...
		for i, child := range item.Children {
			...
			if menu == nil {
				...
				menu = &entity.Menu{
					...
					Path:      child.Path,
					Name:      child.Name,
					Component: jsonconv.AnyToJsonNE(child.Component),
					Redirect:  child.Redirect,
					Type:      child.Type,
					Extra:     jsonconv.AnyToJsonNE(child.Meta),
				}
