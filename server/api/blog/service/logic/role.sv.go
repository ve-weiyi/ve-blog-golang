...
// 分页获取Role记录
func (s *RoleService) GetRoleTreeList(reqCtx *request.Context, page *request.PageInfo) (list []*response.RoleInfo, total int64, err error) {
...
func (s *RoleService) UpdateRoleResources(reqCtx *request.Context, req *request.UpdateRoleResources) (data interface{}, err error) {
	...
	// 查询资源列表
	page := &request.PageInfo{Conditions: []*request.Condition{
		{
			Flag:  "and",
			Field: "api_id",
			Rule:  "in",
