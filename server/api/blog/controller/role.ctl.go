...
// @Success		200		{object}	response.Body{data=entity.Role}	"返回信息"
...
func (s *RoleController) CreateRole(c *gin.Context) {
	...
	data, err := service.NewRoleService(s.svcCtx).CreateRole(reqCtx, &req)
	if err != nil {
		response.ResponseError(c, err)
		return
	}
...
func (s *RoleController) DeleteRoleList(c *gin.Context) {
	...
	data, err := service.NewRoleService(s.svcCtx).DeleteRoleList(reqCtx, &req)
	if err != nil {
		response.ResponseError(c, err)
		return
	}
...
func (s *RoleController) FindRole(c *gin.Context) {
	...
	data, err := service.NewRoleService(s.svcCtx).FindRole(reqCtx, &req)
	if err != nil {
		response.ResponseError(c, err)
		return
	}
...
// @Success		200		{object}	response.Body{data=dto.PageResult{list=[]entity.Role}}	"返回信息"
...
func (s *RoleController) FindRoleList(c *gin.Context) {
	...
	list, total, err := service.NewRoleService(s.svcCtx).FindRoleList(reqCtx, &page)
	if err != nil {
		response.ResponseError(c, err)
		return
	}
