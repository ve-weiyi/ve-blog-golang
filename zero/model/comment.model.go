...
// 切换事务操作
func (s *defaultCommentModel) WithTransaction(tx *gorm.DB) (out CommentModel) {
...
// 分页查询Comment记录
func (s *defaultCommentModel) FindList(ctx context.Context, limit int, offset int, sorts string, conditions string, args ...interface{}) (list []*Comment, err error) {
	// 创建db
	db := s.DbEngin.WithContext(ctx).Table(s.tableName)
