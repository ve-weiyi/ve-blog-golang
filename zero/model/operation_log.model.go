...
// 切换事务操作
func (s *defaultOperationLogModel) WithTransaction(tx *gorm.DB) (out OperationLogModel) {
...
// 分页查询OperationLog记录
func (s *defaultOperationLogModel) FindList(ctx context.Context, limit int, offset int, sorts string, conditions string, args ...interface{}) (list []*OperationLog, err error) {
	// 创建db
	db := s.DbEngin.WithContext(ctx).Table(s.tableName)
