...
// 切换事务操作
func (s *defaultArticleTagModel) WithTransaction(tx *gorm.DB) (out ArticleTagModel) {
...
// 删除ArticleTag记录
func (s *defaultArticleTagModel) Delete(ctx context.Context, id int64) (rows int64, err error) {
	db := s.DbEngin.WithContext(ctx).Table(s.tableName)
...
// 分页查询ArticleTag记录
func (s *defaultArticleTagModel) FindList(ctx context.Context, limit int, offset int, sorts string, conditions string, args ...interface{}) (list []*ArticleTag, err error) {
	// 创建db
	db := s.DbEngin.WithContext(ctx).Table(s.tableName)
