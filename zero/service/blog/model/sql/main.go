...
var tplFile = flag.String("t", "./", "the template file")
var outPath = flag.String("o", "model.tpl", "the output path")
...
func GenerateModel() error {
	...
	f := path.Join(dir, *sqlFile)
	t := path.Join(dir, *tplFile)
	o := path.Join(dir, *outPath)
