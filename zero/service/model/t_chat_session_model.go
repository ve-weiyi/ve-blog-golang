...
type (
	...
	TChatSessionModel interface {
		...
		Inserts(ctx context.Context, in ...*TChatSession) (rows int64, err error)
		// 删除
...
// 插入记录（批量操作）
func (m *defaultTChatSessionModel) Inserts(ctx context.Context, in ...*TChatSession) (rows int64, err error) {
	db := m.DbEngin.WithContext(ctx).Table(m.table)
...
// 删除记录（批量操作）
func (m *defaultTChatSessionModel) Deletes(ctx context.Context, conditions string, args ...interface{}) (rows int64, err error) {
	db := m.DbEngin.WithContext(ctx).Table(m.table)
...
// 更新记录（批量操作）
func (m *defaultTChatSessionModel) Updates(ctx context.Context, columns map[string]interface{}, conditions string, args ...interface{}) (rows int64, err error) {
	db := m.DbEngin.WithContext(ctx).Table(m.table)
