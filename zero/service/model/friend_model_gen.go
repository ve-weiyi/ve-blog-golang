...
type (
	...
	FriendModel interface {
		...
		UpdateNotEmpty(ctx context.Context, in *Friend) (rows int64, err error)
		// 删除
...
// 更新记录（更新零值）
func (m *defaultFriendModel) UpdateNotEmpty(ctx context.Context, in *Friend) (rows int64, err error) {
	db := m.DbEngin.WithContext(ctx).Table(m.table)
