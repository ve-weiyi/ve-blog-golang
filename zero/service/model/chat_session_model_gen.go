...
type (
	...
	ChatSessionModel interface {
		...
		UpdateNotEmpty(ctx context.Context, in *ChatSession) (rows int64, err error)
		// 删除
...
// 更新记录（更新零值）
func (m *defaultChatSessionModel) UpdateNotEmpty(ctx context.Context, in *ChatSession) (rows int64, err error) {
	db := m.DbEngin.WithContext(ctx).Table(m.table)
