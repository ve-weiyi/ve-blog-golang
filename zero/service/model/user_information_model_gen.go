...
type (
	...
	UserInformationModel interface {
		...
		Save(ctx context.Context, in *UserInformation) (rows int64, err error)
		// 删除
...
// 更新记录（更新零值）
func (m *defaultUserInformationModel) Save(ctx context.Context, in *UserInformation) (rows int64, err error) {
	db := m.DbEngin.WithContext(ctx).Table(m.table)
