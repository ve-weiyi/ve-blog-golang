...
type (
	...
	PageModel interface {
		...
		UpdateNotEmpty(ctx context.Context, in *Page) (rows int64, err error)
		// 删除
...
// 更新记录（更新零值）
func (m *defaultPageModel) UpdateNotEmpty(ctx context.Context, in *Page) (rows int64, err error) {
	db := m.DbEngin.WithContext(ctx).Table(m.table)
