...
func applyGenerate(p *spec.ApiSpec, host string, basePath string, schemes string) (*swaggerObject, error) {
	title, _ := strconv.Unquote(p.Info.Properties["title"])
	version, _ := strconv.Unquote(p.Info.Properties["version"])
	desc, _ := strconv.Unquote(p.Info.Properties["desc"])
	...
	renderServiceRoutes(p.Service, p.Service.Groups, s.Paths, requestResponseRefs)

	renderReplyAsDefinition(s.Definitions, p.Types, requestResponseRefs)
