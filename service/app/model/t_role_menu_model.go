...
type (
	...
	TRoleMenuModel interface {
		TableName() string
		// 在事务中操作
		WithTx(tx *gorm.DB) (out TRoleMenuModel)
		// 插入
		Insert(ctx context.Context, in *TRoleMenu) (rows int64, err error)
		InsertBatch(ctx context.Context, in ...*TRoleMenu) (rows int64, err error)
		// 删除
		...
		FindOne(ctx context.Context, sorts string, conditions string, args ...interface{}) (out *TRoleMenu, err error)
	...
	TRoleMenu struct {
		...
		MenuId int64 `gorm:"column:menu_id;type:bigint;not null;default:0;comment:菜单id" json:"menu_id"`  // 菜单id
	}
...
// 在事务中操作
func (m *defaultTRoleMenuModel) WithTx(tx *gorm.DB) (out TRoleMenuModel) {
...
// 插入记录（批量操作）
func (m *defaultTRoleMenuModel) InsertBatch(ctx context.Context, in ...*TRoleMenu) (rows int64, err error) {
	db := m.DbEngin.WithContext(ctx).Table(m.table)
...
// 删除记录（批量操作）
func (m *defaultTRoleMenuModel) DeleteBatch(ctx context.Context, conditions string, args ...interface{}) (rows int64, err error) {
	db := m.DbEngin.WithContext(ctx).Table(m.table)
...
// 更新记录（批量操作）
func (m *defaultTRoleMenuModel) UpdateFields(ctx context.Context, feilds map[string]interface{}, conditions string, args ...interface{}) (rows int64, err error) {
	db := m.DbEngin.WithContext(ctx).Table(m.table)
...
// 查询记录
func (m *defaultTRoleMenuModel) FindOne(ctx context.Context, sorts string, conditions string, args ...interface{}) (out *TRoleMenu, err error) {
	db := m.DbEngin.WithContext(ctx).Table(m.table)
