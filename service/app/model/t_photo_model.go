...
type (
	...
	TPhotoModel interface {
		TableName() string
		// 在事务中操作
		WithTx(tx *gorm.DB) (out TPhotoModel)
		// 插入
		...
		FindOne(ctx context.Context, sorts string, conditions string, args ...interface{}) (out *TPhoto, err error)
	...
	TPhoto struct {
		...
		AlbumId   int64     `gorm:"column:album_id;type:bigint;not null;default:0;comment:相册id" json:"album_id"`                       // 相册id
		PhotoName string    `gorm:"column:photo_name;type:varchar(64);not null;default:'';comment:照片名" json:"photo_name"`              // 照片名
		PhotoDesc string    `gorm:"column:photo_desc;type:varchar(128);not null;default:'';comment:照片描述" json:"photo_desc"`            // 照片描述
		PhotoSrc  string    `gorm:"column:photo_src;type:varchar(255);not null;default:'';comment:照片地址" json:"photo_src"`              // 照片地址
...
// 在事务中操作
func (m *defaultTPhotoModel) WithTx(tx *gorm.DB) (out TPhotoModel) {
...
// 更新记录（批量操作）
func (m *defaultTPhotoModel) UpdateFields(ctx context.Context, feilds map[string]interface{}, conditions string, args ...interface{}) (rows int64, err error) {
	db := m.DbEngin.WithContext(ctx).Table(m.table)
...
// 查询记录
func (m *defaultTPhotoModel) FindOne(ctx context.Context, sorts string, conditions string, args ...interface{}) (out *TPhoto, err error) {
	db := m.DbEngin.WithContext(ctx).Table(m.table)
