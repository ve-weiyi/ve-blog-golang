...
// @Success		200		{object}	response.Body{data=types.UserApisResp}	"返回信息"
...
func (s *UserController) GetUserApis(c *gin.Context) {
	...
	data, err := logic.NewUserLogic(s.svcCtx).GetUserApis(reqCtx, req)
	if err != nil {
		response.ResponseError(c, err)
		return
	}
...
func (s *UserController) GetUserLoginHistoryList(c *gin.Context) {
	...
	data, err := logic.NewUserLogic(s.svcCtx).GetUserLoginHistoryList(reqCtx, req)
	if err != nil {
		response.ResponseError(c, err)
		return
	}
...
// @Produce		application/json
// @Param		data	body		types.EmptyReq		true	"请求参数"
// @Success		200		{object}	response.Body{data=types.UserMenusResp}	"返回信息"
...
func (s *UserController) GetUserMenus(c *gin.Context) {
	...
	data, err := logic.NewUserLogic(s.svcCtx).GetUserMenus(reqCtx, req)
	if err != nil {
		response.ResponseError(c, err)
		return
	}
...
// @Produce		application/json
// @Param		data	body		types.EmptyReq		true	"请求参数"
// @Success		200		{object}	response.Body{data=types.UserRolesResp}	"返回信息"
...
func (s *UserController) GetUserRoles(c *gin.Context) {
	...
	data, err := logic.NewUserLogic(s.svcCtx).GetUserRoles(reqCtx, req)
	if err != nil {
		response.ResponseError(c, err)
		return
	}
