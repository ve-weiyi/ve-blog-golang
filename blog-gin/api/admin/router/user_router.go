...
func (s *UserRouter) Register(r *gin.RouterGroup) {
	...
	// [AdminToken Permission OperationLog]
	{
		...
		// 修改用户头像
		group.PUT("/user/update_user_avatar", h.UpdateUserAvatar)
		// 修改用户绑定邮箱
		group.PUT("/user/update_user_bind_email", h.UpdateUserBindEmail)
		// 修改用户绑定手机号
		group.PUT("/user/update_user_bind_phone", h.UpdateUserBindPhone)
		// 修改用户绑定第三方平台账号
		group.PUT("/user/update_user_bind_third_party", h.UpdateUserBindThirdParty)
		// 修改用户信息
		group.PUT("/user/update_user_info", h.UpdateUserInfo)
		// 修改用户密码
		group.PUT("/user/update_user_password", h.UpdateUserPassword)
	}
}
